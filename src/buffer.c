...
xmlSecBufferCreate(xmlSecSize size) {
    ...
    if(ret < 0) {
        xmlSecInternalError2("xmlSecBufferInitialize", NULL, "size=%d", (int)size);
...
xmlSecBufferSetData(xmlSecBufferPtr buf, const xmlSecByte* data, xmlSecSize size) {
    ...
    if(size > 0) {
        ...
        if(ret < 0) {
            xmlSecInternalError2("xmlSecBufferSetMaxSize", NULL, "size=%d", (int)size);
            return(-1);
        }
...
xmlSecBufferAppend(xmlSecBufferPtr buf, const xmlSecByte* data, xmlSecSize size) {
    ...
    if(size > 0) {
        ...
        if(ret < 0) {
            xmlSecInternalError2("xmlSecBufferSetMaxSize", NULL, "size=%d", (int)(buf->size + size));
            return(-1);
        }
...
xmlSecBufferReadFile(xmlSecBufferPtr buf, const char* filename) {
    ...
    while(!feof(f)) {
        ...
        if(ret < 0) {
            xmlSecInternalError2("xmlSecBufferAppend", NULL, "size=%d", (int)len);
            fclose(f);
            return(-1);
        }
    }
...
xmlSecBufferBase64NodeContentRead(xmlSecBufferPtr buf, xmlNodePtr node) {
    ...
    if(ret < 0) {
        xmlSecInternalError2("xmlSecBufferSetSize", NULL, "size=%d", (int)size);
        xmlFree(content);
        return(-1);
    }
    xmlFree(content);
...
xmlSecBufferIOWrite(xmlSecBufferPtr buf, const xmlSecByte *data, xmlSecSize size) {
    ...
    if(ret < 0) {
        xmlSecInternalError2("xmlSecBufferAppend", NULL, "size=%d", (int)size);
        return(-1);
    }
